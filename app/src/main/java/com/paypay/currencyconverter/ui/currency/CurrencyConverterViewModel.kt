package com.paypay.currencyconverter.ui.currency
...
class CurrencyConverterViewModel internal constructor(private val currencyRepoInterface: CurrencyConverterRepositoryInterface) :
    ...
    private fun handleIntentAction() {
        viewModelScope.launch {
            intentAction.consumeAsFlow().collect {
                when (it) {
                    is ConverterIntent.Fetch -> fetchCurrencyExchangeData()
                    is ConverterIntent.UpdateSpinner -> updateSpinner(it.currencyResponse)
                    is ConverterIntent.Convert -> startConversion(it)
                }
            }
        }
    }
    ...
    private fun getConvertedCurrencyList(it: ConverterIntent.Convert): List<ExchangeRate> {
        it.baseRate?.let { selectedRate.postValue(it) }
        val convertedRateList = ConverterUtil.convertValueAndGet(
            it.baseRate,
            it.amount,
            currencyLiveData.value?.exchangeRateList
        )
